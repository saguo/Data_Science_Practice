...
Num_global_reactive_power <- as.numeric(subSetData$Global_reactive_power)
Str_Datetime <- strptime(paste(subSetData$Date, subSetData$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 
num_voltage <- as.numeric(subSetData$Voltage)
...
# Plot4.1
plot(Str_Datetime, Num_global_active_power, type = "l", xlab = "", ylab = "Global Active Power")

# Plot4.2
plot(Str_Datetime, num_voltage, type = "l", xlab = "datetime", ylab = "Voltage")
...
# Plot4.4
plot(Str_Datetime, Num_global_reactive_power, type = "l", xlab = "datetime", ylab = "Global_reactive_power")
